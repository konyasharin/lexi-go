...
CREATE TABLE "modules_tags" (
	"id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "modules_tags_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"user_id" integer NOT NULL,
	"name" varchar(255) NOT NULL,
...
ALTER TABLE "modules_to_tags" ADD CONSTRAINT "modules_to_tags_tag_id_modules_tags_id_fk" FOREIGN KEY ("tag_id") REFERENCES "public"."modules_tags"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "modules_tags" ADD CONSTRAINT "modules_tags_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "vocabularies" ADD CONSTRAINT "vocabularies_module_id_modules_id_fk" FOREIGN KEY ("module_id") REFERENCES "public"."modules"("id") ON DELETE no action ON UPDATE no action;